...
// default split between the left and right fields
#define kTopSplit		0.7
#define kBottomSplit	0.7

static NSInteger FONT_SIZE			= 12;

@interface QuadFieldTableViewCell (Private)
// Internal methods.
...
@implementation QuadFieldTableViewCell

@synthesize aa, bb, dd, cc, aaColor, bbColor, ccColor, ddColor;
...
+ (UIView *)headerView:(NSArray *)fieldNameArray 
	...
	{
		CGRect rect = CGRectMake(0,0, width, [QuadFieldTableViewCell cellHeight]);
		...
		if (bgColor && bgColor == [UIColor whiteColor]) {
			...
			[QuadFieldTableViewCell customizeLabel:faa bb:fbb cc:fcc dd:fdd];
		} else {
			// Header customized sets the textcolor to be white to show better with a dark background.
			[QuadFieldTableViewCell headerCustomizeLabel:faa bb:fbb cc:fcc dd:fdd];
		}

		[QuadFieldTableViewCell layoutSubviews:faa bb:fbb cc:fcc dd:fdd 
									   inBound:rect 
									  topSplit:top 
								   bottomSplit:bot];
...
+ (UIView *)headerView:(NSArray *)fieldNameArray 
	...
	return [QuadFieldTableViewCell headerView:fieldNameArray boundWidth:width topSplit:top bottomSplit:bot backgroundColor:nil];
}
		 ...
		 accessType:(UITableViewCellAccessoryType) at
{
	if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
		...
		[QuadFieldTableViewCell customizeLabel:aa bb:bb cc:cc dd:dd]; 
...
+ (void) customizeLabel:(UILabel *) aa bb: (UILabel *) bb
	...
	bb.textColor = [UIColor grayColor];		// default gray color
...
- (void)layoutSubviews {
	...
	[QuadFieldTableViewCell layoutSubviews:aa bb:bb cc:cc dd: dd 
...
+ (void) layoutSubviews:(UILabel *) aa bb: (UILabel *) bb
	...
	int leftSection = trunc(baseRect.size.width * topSplit);
    CGRect rect = baseRect;
	rect.origin.y -= 5;
	rect.size.width = leftSection;
	aa.frame = rect;
	rect.origin.x += leftSection-25;
	rect.size.width = baseRect.size.width - leftSection+30;
	cc.frame = rect;

...
+(void)createHand:(NSString *)hand suit1:(UIImageView *)suit1 label1:(UILabel *)label1 suit2:(UIImageView *)suit2 label2:(UILabel *)label2 {
	...
	if(cards.count>1) {
		...
		if(card1.length==2 && card2.length==2) {
			[BigHandObj createCard:suit1 label:label1 card:[card1 substringToIndex:1] suit:[card1 substringFromIndex:1]];
			[BigHandObj createCard:suit2 label:label2 card:[card2 substringToIndex:1] suit:[card2 substringFromIndex:1]];
		}
	}
}
